...
// .wrangler/tmp/bundle-9exoOq/checked-fetch.js
...
// .wrangler/tmp/bundle-9exoOq/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-9exoOq/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-9exoOq/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var createSupabaseClient = /* @__PURE__ */ __name((env) => {
  const url = env?.SUPABASE_URL || "https://cihuylmusthumxpuexrl.supabase.co";
  const key = env?.SUPABASE_SERVICE_ROLE_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNpaHV5bG11c3RodW14cHVleHJsIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MjU2OTExMSwiZXhwIjoyMDY4MTQ1MTExfQ.2sdXqL8HuvZris4IRzP0uG-yLyUIlgVyWwrKv6MpLTQ";
  if (!url || !key) {
...
var nanoid2;
(async () => {
  nanoid2 = (await Promise.resolve().then(() => (init_index_browser(), index_browser_exports))).nanoid;
})();
var receipts = new Hono2();
var receiptSchema = external_exports.object({
  store_name: external_exports.string(),
  total_amount: external_exports.number(),
...
receipts.post("/", zValidator("json", receiptSchema), async (c) => {
  ...
  let { data: store, error: storeError } = await supabaseClient.from("stores").select("id").ilike("name", receiptData.store_name).single();
  if (storeError && storeError.code !== "PGRST116") {
    return c.json({ error: "Error fetching store" }, 500);
  }
  if (!store) {
    const { data: newStore, error: newStoreError } = await supabaseClient.from("stores").insert({ name: receiptData.store_name }).select("id").single();
    if (newStoreError) {
      return c.json({ error: "Error creating store" }, 500);
    }
    store = newStore;
  }
  const { data: newReceipt, error: receiptError } = await supabaseClient.from("receipts").insert({
    user_id: user.id,
    store_id: store.id,
  ...
  const { error: itemsError } = await supabaseClient.from("receipt_items").insert(itemsToInsert);
  if (itemsError) {
...
receipts.get("/", async (c) => {
  const user = c.get("user");
  const supabaseClient = createSupabaseClient(c.env);
  const { data, error } = await supabaseClient.from("receipts").select(
    `
      id,
      date,
      total_amount,
...
receipts.get("/upload-url", async (c) => {
  ...
  const key = `${user.id}/${(await Promise.resolve().then(() => (init_index_browser(), index_browser_exports))).nanoid()}.jpg`;
  const presignedUrl = "https://your-r2-bucket-url.com/" + key;
  return c.json({
    url: presignedUrl,
    key
  });
...
// src/routes/households.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
var households = new Hono2();
households.post(
  "/",
  zValidator(
    "json",
    external_exports.object({
      name: external_exports.string().min(1, "Household name cannot be empty.")
    })
  ),
  async (c) => {
    ...
    const user = c.get("user");
    const supabaseClient = createSupabaseClient(c.env);
    const { data: existingUser, error: userCheckError } = await supabaseClient.from("users").select("id").eq("id", user.id).single();
    if (userCheckError && userCheckError.code === "PGRST116") {
      const { error: createUserError } = await supabaseClient.from("users").insert({
        id: user.id,
        email: user.email
      });
      if (createUserError) {
    ...
    const { data: household, error } = await supabaseClient.from("households").insert({ name, owner_id: user.id }).select().single();
    if (error) {
      return c.json({ error: error.message }, 500);
    }
    const { error: memberError } = await supabaseClient.from("household_users").insert({ household_id: household.id, user_id: user.id });
    if (memberError) {
...
households.get("/mine", async (c) => {
  ...
  const { data, error } = await supabaseClient.from("households").select(
    ...
    `
  ).eq("members.user_id", user.id).single();
  if (error) {
...
households.post(
  ...
  async (c) => {
    ...
    const supabaseClient = createSupabaseClient(c.env);
    const { data: household, error: householdError } = await supabaseClient.from("households").select("owner_id").eq("id", householdId).eq("owner_id", requester.id).single();
    if (householdError || !household) {
      return c.json({ error: "Unauthorized or household not found" }, 403);
    }
    const { data: newMember, error: userError } = await supabaseClient.from("users").select("id").eq("email", newMemberEmail).single();
    ...
    const { error: insertError } = await supabaseClient.from("household_users").insert({ household_id: householdId, user_id: newMember.id });
    if (insertError) {
...
households.get("/:id", async (c) => {
  ...
  const { data, error } = await supabaseClient.from("households").select(
    `
      ...
      members:household_users(user:users(id, email))
    `
  ).eq("id", id).eq("members.user_id", user.id).single();
  if (error) {
    return c.json(
...
analytics.get("/spending", async (c) => {
  ...
  const supabaseClient = createSupabaseClient(c.env);
  try {
    const { data: totalSpending, error: totalError } = await supabaseClient.from("receipts").select("total_amount").eq("user_id", user.id);
    if (totalError) {
      return c.json({ error: "Failed to fetch spending data" }, 500);
    }
    const total = totalSpending?.reduce((sum, receipt) => sum + receipt.total_amount, 0) || 0;
    const { data: categorySpending, error: categoryError } = await supabaseClient.from("receipts").select(
      `
        total_amount,
        ocr_data,
        category:categories(name, color)
      `
    ).eq("user_id", user.id).not("ocr_data", "is", null);
    if (categoryError) {
      return c.json({ error: "Failed to fetch category data" }, 500);
    }
    const categoryBreakdown = /* @__PURE__ */ new Map();
    ...
    const { data: weeklySpending, error: weeklyError } = await supabaseClient.from("receipts").select("total_amount, date").eq("user_id", user.id).gte(
      "date",
...
analytics.get("/price-comparison/:item", async (c) => {
  ...
  const supabaseClient = createSupabaseClient(c.env);
  try {
    const { data: priceHistory, error: priceError } = await supabaseClient.from("price_history").select(
      `
        price,
        date,
        store:stores(name, location)
      `
    ).ilike("item_name", `%${itemName}%`).order("date", { ascending: false }).limit(50);
    if (priceError) {
      return c.json({ error: "Failed to fetch price data" }, 500);
    }
    const storePrices = /* @__PURE__ */ new Map();
...
analytics.get("/price-history/:item", async (c) => {
  ...
  const supabaseClient = createSupabaseClient(c.env);
  try {
    const { data: priceHistory, error } = await supabaseClient.from("price_history").select(
      `
        price,
        date,
        store:stores(name)
      `
    ).ilike("item_name", `%${itemName}%`).order("date", { ascending: true }).limit(100);
    if (error) {
...
analytics.get("/stores", async (c) => {
  ...
  const supabaseClient = createSupabaseClient(c.env);
  try {
    const { data: storeSpending, error } = await supabaseClient.from("receipts").select("store_name, total_amount, date").eq("user_id", user.id).order("date", { ascending: false });
    if (error) {
      return c.json({ error: "Failed to fetch store data" }, 500);
    }
    const storeAnalytics = /* @__PURE__ */ new Map();
    ...
    const analytics2 = Array.from(storeAnalytics.entries()).map(
      ([storeName, data]) => ({
        storeName,
        ...data
      })
    );
    analytics2.sort((a, b) => b.totalSpent - a.totalSpent);
    return c.json(analytics2);
  } catch (error) {
...
analytics.get("/savings", async (c) => {
  ...
  const supabaseClient = createSupabaseClient(c.env);
  try {
    const { data: receipts2, error } = await supabaseClient.from("receipts").select("savings_identified, cashback_earned, total_amount, date").eq("user_id", user.id).not("savings_identified", "is", null);
    if (error) {
      return c.json({ error: "Failed to fetch savings data" }, 500);
    }
    const totalSavings = receipts2?.reduce(
      (sum, receipt) => sum + (receipt.savings_identified || 0),
      0
    ) || 0;
    const totalCashback = receipts2?.reduce(
      (sum, receipt) => sum + (receipt.cashback_earned || 0),
      0
    ) || 0;
    const totalSpent = receipts2?.reduce((sum, receipt) => sum + receipt.total_amount, 0) || 0;
    return c.json({
      totalSavings,
      totalCashback,
      totalSpent,
      savingsPercentage: totalSpent > 0 ? totalSavings / totalSpent * 100 : 0,
      receiptCount: receipts2?.length || 0,
      averageSavings: receipts2?.length ? totalSavings / receipts2.length : 0
    });
  } catch (error) {
